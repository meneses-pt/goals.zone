...
class MatchesHistoryListView(generic.ListView):
    ...
    def get_queryset(self) -> QuerySet:
        ...
        return (
            Match.objects.filter(datetime__gte=start_date, datetime__lt=end_date, videogoal__isnull=False)
            .distinct()
            .order_by("datetime")
        )
...
class MatchWeekSearchView(generics.ListAPIView):
    ...
    def get_queryset(self) -> QuerySet:
        ...
        start_date = localize_date(start_date)
        queryset = (
            Match.objects.filter(datetime__gte=start_date, videogoal__isnull=False).distinct().order_by("-datetime")
        )
